...
def run_analysis_pipeline(parsed_report):
    ...
    if not abnormal_tests:
        return {
            "status": "all_normal",
            "message": "All values are within normal range. Great health!",
            "abnormal_values": [],
            "specialist": None,
            "questions": []
        }

    print(f"[Pipeline] Found {len(abnormal_tests)} abnormal values")
...
def recommend_specialist(llm, analysis, knowledge):
    prompt = PromptTemplate(
        ...
        template="""
Help a patient find the right doctor based on their abnormal lab results.
...
{{
    ...
    "booking_message": "what to say when booking appointment",
...
Rules:
- Reference their ACTUAL values
- Cover diagnosis, treatment, lifestyle, diet, and follow-up
